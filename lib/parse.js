...
module.exports.connect = function(buf, packet) {
  ...
  if(pos > len) return new Error('Parse error - position after protocol parsing is bigger than overall length');
  ...
  if(packet.keepalive === null) return new Error('Parse error - cannot parse packet keepalive');
  pos += 2;
  ...
  if(flags.password) {
    ...
    if(password === null) return new Error('Parse error - cannot parse password');
